...
let unary_fun f = PrimFuncVal (fun vals loc -> match vals with
    | [a] -> f a loc
    | _ -> panic ())

let binary_fun f = PrimFuncVal (fun vals loc -> match vals with
    | [a; b] -> f a b loc
    | _ -> panic ())

(* Integer operations. *)

let binary_int_fun f = binary_fun (fun a b loc -> match (a, b) with
...
let is_nil = unary_fun (fun v loc -> match v with
    | NilVal -> BoolVal true
...
let safe_div a b =
  try
    a / b
  with Division_by_zero -> Error.runtime_err "division by zero"
